# remote-agent-lite server

面向 2C2G 云服务器的轻量远程 Codex 工作台的服务端部分：常驻 `codex app-server`，通过 FastAPI 提供单管理员 Web 界面。手机和电脑使用同一个响应式页面，可以创建/删除项目、上传/下载文件、管理多个会话（重命名、置顶、复制、删除），并只接收当前打开会话的流式消息。

本文件里的命令都在本目录（`remote-agent-server/`）里执行；其它端的说明见仓库根目录的 [README](../README.md)。
